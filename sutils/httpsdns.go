...
func NewGoogleHttpsDns() (handler *GoogleHttpsDns, err error) {
	handler = &GoogleHttpsDns{
		baseurl:   "https://dns.google.com/resolve",
		transport: &http2.Transport{},
	}
	handler.Lookuper = &ExchangerToLookuper{
		Exchanger: handler,
	}

	jsonresp, err := handler.QueryHttpsDNS("1", "www.google.com", "114.114.114.114")
	if err != nil {
		logger.Error("warmup err: %s.", err.Error())
		return
	}

	for _, a := range jsonresp.Answer {
		logger.Debug("google result: %s.", a.Data)
	}
	return
}

func (handler *GoogleHttpsDns) Exchange(quiz *dns.Msg) (resp *dns.Msg, err error) {
	var subnet string
...
func (handler *GoogleHttpsDns) QueryHttpsDNS(qtype, name, subnet string) (jsonresp *DNSMsg, err error) {
	req, err := http.NewRequest("GET", handler.baseurl, nil)
	if err != nil {
		logger.Error("fail to create request: %s.", err.Error())
		return
	}
	...
	if err != nil {
		logger.Error("https err:", err.Error())
		return
	}
	...
	if err != nil {
		logger.Error("decode err:", err.Error())
		return
	}
	return
}
